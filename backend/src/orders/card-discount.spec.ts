...
describe('resolveAutoDiscount — card-linked discount gate', () => {
  ...
  const makeService = (discounts: unknown[]) => {
    ...
    svc.branchRepo = { findOne: jest.fn(async () => ({ timezone: 'Asia/Karachi' })) };
    return svc as {
      resolveAutoDiscount: (
        tenantId: number,
        subtotal: number,
  ...
  const lines = [{ menuItemId: 1, categoryId: 1, itemSubtotal: 1000, quantity: 1 }];
  const run = (fullCard: boolean, bankCardId: number | null) =>
    makeService([cardDiscount]).resolveAutoDiscount(1, 1000, 'pos', 10, 100, lines, fullCard, bankCardId);
  ...
  it('a non-card discount still applies regardless of tender', async () => {
    const svc = Object.create(OrdersService.prototype);
    svc.discountResultEmpty = {
      discountAmount: 0, discountCode: null, discountId: null,
      scope: 'whole_order', scopeIds: [], discountableAmount: 0, eligibilityBrandIds: null,
    };
    svc.discountRepo = { find: jest.fn(async () => [{ ...cardDiscount, requiresCard: false, eligibleBankCardIds: null }]) };
    svc.branchRepo = { findOne: jest.fn(async () => ({ timezone: 'Asia/Karachi' })) };
    const res = await (svc as { resolveAutoDiscount: (...a: unknown[]) => Promise<{ discountAmount: number }> })
