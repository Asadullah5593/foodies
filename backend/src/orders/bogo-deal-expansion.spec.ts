...
describe('expandDealItems — BOGO deal', () => {
    ...
    items.set(
        301,
        mkItem(301, 2, null, 699, [{ id: 3012, sizeKey: '12', mod: 1250 }]),
    );
    items.set(
        302,
        mkItem(302, 2, null, 699, [{ id: 3022, sizeKey: '12', mod: 1250 }]),
    );
    // Item NOT enrolled in the deal (not in SOURCE_IDS) but with a 12" variant.
    items.set(
        401,
        mkItem(401, 1, 'Classic', 5000, [{ id: 4012, sizeKey: '12', mod: 0 }]),
    );
    // BOGO deal root
    items.set(999, {
        ...mkItem(999, 9, null, 0, []),
    ...
    it('ordering the deal x2 expands two independent priced sets', async () => {
        ...
        expect(out.map((o) => o.deal_unit_price)).toEqual([
            1749, 649.5, 1749, 649.5,
        ]);
    });
});
