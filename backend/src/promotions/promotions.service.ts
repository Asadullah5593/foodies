...
export class PromotionsService {
    ...
    private async deactivateAssignments(promotionId: number): Promise<void> {
        await this.dataSource.transaction(async (manager) => {
            // Same lock as claimPromotion: a claim in flight completes first (and its
            // fresh coupon is then disabled below), and a claim starting after this
            // finds every copy already 'expired' and is refused. Bulk atomic UPDATEs
            // replace the stale read-modify-write loop (no lost updates).
            await advisoryXactLock(
                manager,
                AdvisoryLock.PROMOTION,
                promotionId,
            );
            // Disable coupons generated by already-claimed copies so they can no
            // longer be redeemed. Done first so a copy claimed moments ago is caught.
            await manager.query(
                `UPDATE discounts SET is_active = false
                 WHERE id IN (
                   SELECT discount_id FROM customer_promotions
                   WHERE promotion_id = $1 AND status = 'claimed'
                     AND discount_id IS NOT NULL
                 )`,
                [promotionId],
            );
            // Expire every not-yet-used copy (pending or claimed); 'used' copies are
            // left untouched to preserve redemption history.
            await manager.query(
                `UPDATE customer_promotions
                 SET status = 'expired', expires_at = COALESCE(expires_at, now())
                 WHERE promotion_id = $1 AND status IN ('pending', 'claimed')`,
                [promotionId],
            );
        });
    }
    ...
    async claimPromotion(customerPromotionId: number, customerId: number) {
        ...
        return this.dataSource.transaction(async (manager) => {
            // Coordinate with deactivateAssignments on the same promotion lock, so a
            // claim and a promotion switch-off cannot interleave into an orphaned
            // live coupon.
            await advisoryXactLock(manager, AdvisoryLock.PROMOTION, promo.id);
            // Atomic single-claim: only ONE concurrent claim flips pending -> claimed,
            // so a double-tap cannot mint two live coupons from one promotion.
            const prev = await transitionStatus(
                manager,
                'customer_promotions',
                cp.id,
                'claimed',
                { allowedFrom: ['pending'], set: { claimed_at: raw('now()') } },
            );
            if (prev === null) {
                const fresh = await manager
                    .getRepository(CustomerPromotion)
                    .findOne({
                        where: { id: cp.id },
            ...
            // Mint the coupon. Retry on a code collision (the uniqueness probe in
            // generateCouponCode is racy; the unique index on discounts.code is the
            // real guard and we regenerate on 23505).
            let code = '';
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                ...
                try {
                    discount = await manager.getRepository(Discount).save(
                        manager.getRepository(Discount).create({
            ...
            await manager
                ...
                .update(
                    { id: cp.id },
                    { couponCode: code, discountId: discount.id },
                );
