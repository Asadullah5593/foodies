...
const TOP_IT_OFF = ['Chilli Oil', 'Garlic Oil', 'Chilli Flakes', 'Crispy Onion', 'Oregano'];
const BASE_SAUCES = [
    'Tomato Base',
    'Spicy Tomato Base',
    'BBQ Base',
...
const DIPS = ['Ranch', 'Garlic & Herb', 'BBQ', 'Sweet Chilli', 'Tomato Ketchup', 'Hot Peri-Peri'];
const SOFT_DRINKS = ['Coca-Cola', 'Diet Coke', 'Coca-Cola Zero', 'Fanta Orange', 'Sprite'];
// "Add a dip(s)" on a pizza: 1 free, then a tiered bundle price for extra dips.
const DIP_PRICE_TIERS = { '1': 99, '2': 169, '3': 249 };
const WING_FLAVOURS = ['Plain', 'BBQ Crunch', 'Chilli Honey', 'Garlic Parmesan', 'Heat Extreme', 'Peri Peri'];
...
function slugify(name: string) {
    return `${name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}-${BRAND_SLUG}`;
}
...
async function seed() {
    ...
    type SizeDef = { name: string; sizeKey: string | null; price: number; isDefault?: boolean };
    let itemSort = 0;
    ...
    type ModDef = { name: string; price?: number; priceBySize?: Record<string, number> | null; availableForSizes?: string[] | null };
    const mkGroup = async (
        name: string,
        cfg: { minSelect?: number; maxSelect?: number; includedQuantity?: number; includedBySize?: Record<string, number> | null; allowQuantity?: boolean; priceTiers?: Record<string, number> | null },
        mods: ModDef[],
    ...
    const grpCrust = await mkGroup('Choose your Crust', { minSelect: 1, maxSelect: 1 }, [
    ...
    const grpCheeseBYO = await mkGroup('Choose Your Cheese', { minSelect: 1, maxSelect: 2 }, [
        { name: 'Mozzarella' },
        { name: 'Extra Cheese', price: CHEESE_PRICE_BY_SIZE['7'], priceBySize: CHEESE_PRICE_BY_SIZE },
    ]);
    // Classic/Signature cheese: As It Comes / No Cheese / Extra Cheese (per size).
    const grpCheeseSig = await mkGroup('Choose your Cheese', { minSelect: 0, maxSelect: 2 }, [
        { name: 'As It Comes' },
    ...
    const grpMeatBYO = await mkGroup(
        'Choose Your Meat',
        { minSelect: 0, maxSelect: REPEAT_MAX, includedBySize: INCLUDED_BY_SIZE },
        MEATS.map((m) => ({ name: m, price: MEAT_PRICE_BY_SIZE['7'], priceBySize: MEAT_PRICE_BY_SIZE })),
    );
    const grpVegBYO = await mkGroup(
        'Choose Your Veggies',
        { minSelect: 0, maxSelect: REPEAT_MAX, includedBySize: INCLUDED_BY_SIZE },
        VEGGIES.map((v) => ({ name: v, price: VEG_PRICE_BY_SIZE['7'], priceBySize: VEG_PRICE_BY_SIZE })),
    );
    const grpTopItOff = await mkGroup('Top It Off', { minSelect: 0, maxSelect: 1 }, TOP_IT_OFF.map((t) => ({ name: t })));
    ...
    const grpSigToppings = await mkGroup(
        ...
        [
            ...MEATS.map((m) => ({ name: `Extra ${m}`, price: MEAT_PRICE_BY_SIZE['7'], priceBySize: MEAT_PRICE_BY_SIZE })),
            ...VEGGIES.map((v) => ({ name: `Extra ${v}`, price: VEG_PRICE_BY_SIZE['7'], priceBySize: VEG_PRICE_BY_SIZE })),
        ],
    );
    const grpPizzaOrCalzone = await mkGroup('Pizza or Calzone', { minSelect: 1, maxSelect: 1 }, [
        { name: 'Pizza' },
        { name: 'Calzone' },
    ]);
    // Dips on a pizza: 1 free, extra Rs99 each.
    // Dips on a pizza: 1 free, then a tiered bundle price (1=99, 2=169, 3=249) for the extras.
    const grpAddDips = await mkGroup('Add a dip(s)', { minSelect: 0, maxSelect: 6, includedQuantity: 1, priceTiers: DIP_PRICE_TIERS }, DIPS.map((d) => ({ name: d })));
    // Drinks as a paid up-sell. 1L/1.5L are per-flavour so the customer picks which soft drink.
    const grpAddDrinks = await mkGroup('Add a drink(s)', { minSelect: 0, maxSelect: REPEAT_MAX }, [
    ...
    const catWraps = await mkCategory('Firey Wraps', 'All wraps come with lettuce, onion, cucumber and mayo');
    ...
    const pizzaSizes = (p7: number, p10: number, p12: number, p14: number): SizeDef[] => [
    ...
    await linkGroups(byo, [grpCrust, grpBase, grpCheeseBYO, grpMeatBYO, grpVegBYO, grpTopItOff, grpAddDips, grpAddDrinks]);
    ...
    const pizzaGroups = [grpPizzaOrCalzone, grpCrust, grpCheeseSig, grpSigToppings, grpAddDips, grpAddDrinks];

    const classics: Array<[string, string]> = [
        ['Twisted Hawaiian Pizza', 'Tomato base, mozzarella, chicken and pineapple'],
        ['Veggie Supreme Pizza', 'Tomato base, mozzarella, onions, mixed peppers, sweetcorn, mushrooms and green olives'],
        ['BBQ Chicken Pizza', 'BBQ base, mozzarella, red onions, sweetcorn and BBQ chicken'],
        ['Chicken Special Pizza', 'Tomato base, mozzarella, chicken tikka, chicken pepperoni, jalapeno, sweetcorn'],
        ['Chicken Tikka Pizza', 'Tomato base, mozzarella, chicken tikka, mixed peppers, onions and jalapeños'],
    ];
    for (const [name, desc] of classics) {
        const it = await mkItem({ category: catPizza, name, description: desc, basePrice: 659, sizes: classicSizes() });
        await linkGroups(it, pizzaGroups);
    }
    const signatures: Array<[string, string]> = [
        ['Fireaway Special Pizza', 'Spicy tomato base, mozzarella, chicken muglai, chicken, peri peri chicken, red onions, mushrooms, onions'],
        ['King Pepperoni Pizza', 'Tomato base, mozzarella and loaded with pepperoni'],
        ['Chicken Muglai Pizza', 'Tomato base, mozzarella, chicken muglai, chicken fajita, onion and jalapeños'],
        ['Wild Beef Pizza', 'Tomato base, sausages, beef pepperoni, onion, jalapenos'],
        ['Meat Heaven Pizza', 'Tomato base, mozzarella, chicken fajita, sausages, chicken pepperoni, onion, green peppers'],
        ['Sausage Special Pizza', 'Tomato base, mozzarella, sausages, onions, jalapeños and mushrooms'],
        ['Peri Peri Special', 'Peri peri tomato base, mozzarella, peri peri chicken, jalapenos, chillies'],
    ];
    for (const [name, desc] of signatures) {
        const it = await mkItem({ category: catPizza, name, description: desc, basePrice: 699, sizes: signatureSizes() });
        await linkGroups(it, pizzaGroups);
    }
    ...
    const kids = await mkItem({
        ...
        description: 'Margherita pizza for kids in a smiley bear shape: tomato base, mozzarella and fresh basil. Includes a juice drink.',
        basePrice: 899,
        sizes: [{ name: 'Regular', sizeKey: '7', price: 899, isDefault: true }],
    });
    const grpKidsCheese = await mkGroup('Choose your Cheese', { minSelect: 1, maxSelect: 1 }, [
        { name: 'As It Comes' },
        { name: 'No Cheese On The Pizza' },
    ]);
    await linkGroups(kids, [grpKidsCheese, grpSigToppings]);
    const addonKidsFries = await addonRepo.save(
        addonRepo.create({ brandId, categoryId: catKids.id, name: 'Add Fries', price: 199, isActive: true, sortOrder: 0 }),
    );
    await linkAddons(kids, [addonKidsFries]);

    // ——— PASTA (Rs949 each) ———
    const grpAddParmesan = await mkGroup('Additional', { minSelect: 0, maxSelect: 1 }, [{ name: 'Add Parmesan' }]);
    const pastas: Array<[string, string]> = [
        ['Penne Marinara', 'Penne pasta smothered in a tangy marinara sauce, topped with parmesan and fresh basil.'],
        ['Creamy Pesto', 'Pasta smothered in a creamy pesto sauce, topped with parmesan and fresh basil.'],
        ['Mac & Cheese', 'Creamy macaroni pasta smothered in a rich cheese blend, topped with parmesan and mozzarella.'],
        ['Smooth Carbonara', 'Pasta tossed with a creamy carbonara sauce, topped with savory turkey bacon and parmesan.'],
        ['Chicken Arrabiata', 'Tender chicken, earthy mushrooms, and tangy tomatoes tossed with pasta in a flavourful herb-infused sauce.'],
    ];
    const pastaItems: MenuItem[] = [];
    for (const [name, desc] of pastas) {
        const it = await mkItem({ category: catPasta, name, description: desc, basePrice: 949 });
        await linkGroups(it, [grpAddParmesan, grpAddDrinks]);
        pastaItems.push(it);
    }

    // ——— CLASSIC SIDES ———
    await mkItem({ category: catSides, name: 'Garlic Bread', description: 'Freshly baked 10" base topped with our famous garlic oil', basePrice: 699 });
    // Cheesy garlic bread toppings: single size → flat extra meat Rs149 / veg Rs69.
    const grpSideToppings = await mkGroup('Add Extra Toppings', { minSelect: 0, maxSelect: REPEAT_MAX }, [
    ...
    const cheesyGarlicBread = await mkItem({
        ...
        description: 'Freshly baked 10" base topped with our famous garlic oil and 100% mozzarella cheese',
        basePrice: 799,
    });
    ...
    const wedges = await mkItem({
        ...
        description: 'Homemade British potato wedges with garlic, herbs and the option of parmesan cheese',
        basePrice: 499,
    });
    await linkGroups(wedges, [grpAddParmesan]);
    const grpBitesFlavour = await mkGroup('Choose your Flavour', { minSelect: 1, maxSelect: 1 }, [
        { name: 'Chicken Tikka' },
        { name: 'Pepperoni' },
        { name: 'Jalapenos' },
        { name: 'Chicken Fajita' },
    ]);
    const bites = await mkItem({ category: catSides, name: 'Fireaway Bites', description: '6 dough bites', basePrice: 999 });
    ...
    await mkItem({ category: catSides, name: 'Cheesy Garlic Fries', description: 'Popular chips with garlic cheese', basePrice: 499 });
    const grpDirtyCheese = await mkGroup('Choose your Cheese', { minSelect: 1, maxSelect: 1 }, [
        { name: 'As It Comes' },
        { name: 'No Cheese' },
    ]);
    ...
    // ——— WINGS, STRIPS & SALADS ———
    const grpWingFlavour = await mkGroup('Choose your Flavour', { minSelect: 1, maxSelect: 1 }, WING_FLAVOURS.map((f) => ({ name: f })));
    ...
    const strips = await mkItem({
        ...
        description: '100% chicken breast breaded with our own spice blend, available in a choice of 5 flavours',
        basePrice: 799,
        sizes: [
            { name: '5 Pcs', sizeKey: null, price: 799, isDefault: true },
            { name: '10 Pcs', sizeKey: null, price: 1499 },
    ...
    const grpSaladMeat = await mkGroup(
        ...
        ['Chicken Muglai', 'Chicken Fajita', 'Chicken Tikka', 'Peri Peri Chicken'].map((m) => ({ name: m, price: 149 })),
    );
    const grpSaladVeg = await mkGroup(
        'Choose Your Veggies',
        { minSelect: 0, maxSelect: 6, includedQuantity: 4 }, // up to 4 included, extra veg +29
        ['Black Olives', 'Onion', 'Sweet Corn', 'Jalapeños', 'Tomato', 'Mixed Peppers'].map((v) => ({ name: v, price: 29 })),
    );
    const salad = await mkItem({
        category: catWings,
        name: 'Fireaway Special Salad',
        description: 'Oven baked chicken strips or any meat of your choice on a bed of mixed salad, with pineapple, tomatoes, jalapenos, red onion, drizzled with choice of dressing',
        basePrice: 899,
    });
    await linkGroups(salad, [grpSaladMeat, grpSaladVeg]);

    // ——— DIPS (standalone) ———
    const grpPick1Dip = await mkGroup('Choose your dip', { minSelect: 1, maxSelect: 1 }, DIPS.map((d) => ({ name: d })));
    const grpPick2Dip = await mkGroup('Choose 2 dips', { minSelect: 2, maxSelect: 2 }, DIPS.map((d) => ({ name: d })));
    const grpPick3Dip = await mkGroup('Choose 3 dips', { minSelect: 3, maxSelect: 3 }, DIPS.map((d) => ({ name: d })));
    const single = await mkItem({ category: catDips, name: 'Single Dip', description: 'Choose from: Ranch, Garlic & Herb, BBQ, Sweet Chilli, Tomato Ketchup, Hot Peri-Peri', basePrice: 99 });
    await linkGroups(single, [grpPick1Dip]);
    const twoDips = await mkItem({ category: catDips, name: '2 Dips', description: 'Choose 2 dips', basePrice: 169 });
    await linkGroups(twoDips, [grpPick2Dip]);
    const threeDips = await mkItem({ category: catDips, name: '3 Dips', description: 'Choose 3 dips', basePrice: 249 });
    await linkGroups(threeDips, [grpPick3Dip]);

    // ——— FIREY WRAPS ———
    const grpRemoveFilling = await mkGroup('Remove a filling', { minSelect: 0, maxSelect: 4 }, ['Mixed Leaves', 'Onion', 'Peppers', 'Mayo'].map((f) => ({ name: f })));
    // Free, but the same sauce can be added multiple times (extra BBQ ×2) → allowQuantity.
    const grpAddSauce = await mkGroup('Add a Sauce', { minSelect: 0, maxSelect: REPEAT_MAX, allowQuantity: true }, ['Mayo', 'BBQ', 'Ketchup', 'Sweet Chilli'].map((s) => ({ name: s })));
    const addonMakeMeal = await addonRepo.save(
        addonRepo.create({ brandId, categoryId: catWraps.id, name: 'Make it a meal (Reg Fries + 345ml drink)', price: 350, isActive: true, sortOrder: 0 }),
    );
    const wraps: Array<[string, string, number]> = [
        ['Chicken Tikka Wrap', 'Chicken tikka, lettuce, onion, cucumber and mayo', 549],
        ['Krunchy Chicken Wrap', 'Krunchy chicken, lettuce, onion, cucumber and mayo', 549],
        ['Peri Peri Chicken Wrap', 'Peri peri chicken, lettuce, onion, cucumber and mayo', 549],
        ['Chicken and Pepperoni Wrap', 'Chicken & pepperoni, lettuce, onion, cucumber and mayo', 549],
        ['Firey Special Wrap', 'Chicken fajita, kebab, sausage, lettuce, onion, cucumber and mayo', 649],
    ];
    let firySpecialWrap: MenuItem | null = null;
    const allWraps: MenuItem[] = [];
    for (const [name, desc, price] of wraps) {
        const it = await mkItem({ category: catWraps, name, description: desc, basePrice: price });
    ...
    // ——— DESSERTS ———
    const grpChocCrust = await mkGroup('Crust', { minSelect: 1, maxSelect: 1 }, [{ name: 'Thin Crust' }, { name: 'Regular' }]);
    const grpChocToppings = await mkGroup('Additional Toppings', { minSelect: 0, maxSelect: REPEAT_MAX }, [
    ...
    const chocPizza = await mkItem({
        ...
        description: 'Freshly baked 9" pizza base topped with creamy chocolate and icing sugar, and if you dare, loaded with toppings of your choice.',
        basePrice: 999,
    });
    await linkGroups(chocPizza, [grpChocCrust, grpChocToppings]);
    const grpCookieFlavour = await mkGroup('Choose Your Flavour', { minSelect: 1, maxSelect: 1 }, ['Chocolate Cookies', 'Triple Chocolate', 'Pistachio', 'Lotus', 'Red Velvet'].map((f) => ({ name: f })));
    const cookie = await mkItem({ category: catDesserts, name: 'Homemade Cookie', description: 'Choose from our special flavours', basePrice: 350 });
    await linkGroups(cookie, [grpCookieFlavour]);
    const grpSpread = await mkGroup('Choose Your Flavour', { minSelect: 1, maxSelect: 1 }, [{ name: 'With Chocolate Spread' }, { name: 'Without Chocolate Spread' }]);
    const brownie = await mkItem({ category: catDesserts, name: 'Chocolate Brownie', description: 'Chocolate brownie with chocolate spread (1 per portion)', basePrice: 299 });
    await linkGroups(brownie, [grpSpread]);
    const doughBalls = await mkItem({ category: catDesserts, name: 'Chocolate Dough Balls', description: 'Fireaway special 6 chocolate dough balls', basePrice: 899 });
    await linkGroups(doughBalls, [grpSpread]);
    const grpDoughnutFlavour = await mkGroup('Choose Your Flavour', { minSelect: 1, maxSelect: 1 }, ['Plain Glazed Doughnut', 'Boston Cream', 'Oreo', 'Lotus', 'Plain Chocolate', 'Ferrero Rocher', 'Nutella', 'Orange Cream', 'Red Velvet'].map((f) => ({ name: f })));
    const doughnut = await mkItem({ category: catDesserts, name: 'Doughnut', description: 'Our special selection of doughnuts', basePrice: 250 });
    await linkGroups(doughnut, [grpDoughnutFlavour]);

    // ——— MILKSHAKES (Rs499 each) ———
    const shakes = ['Vanilla', 'Oreo', 'Nutella', 'Lotus Biscoff', 'Banana', 'Strawberry', 'Bubble-gum', 'Raspberry', 'Chocolate', 'Pistachio', 'Mango'];
    for (const s of shakes) {
        await mkItem({ category: catShakes, name: `${s} Milkshake`, description: 'Freshly made luscious milkshake with real fresh ingredients', basePrice: 499 });
    }
    ...
    for (const flavour of SOFT_DRINKS) {
        for (const [size, price] of SOFT_DRINK_SIZES) {
            await mkItem({ category: catDrinks, name: `${flavour} ${size}`, basePrice: price });
        }
    }
    ...
    for (const n of ['Coca-Cola 250ml', 'Diet Coke 250ml', 'Coca-Cola Zero 250ml', 'Fanta Orange 250ml', 'Sprite 250ml', 'Still Water 250ml']) {
        deal250Drinks.push(await mkItem({ category: catDrinks, name: n, basePrice: 0, dealOnly: true }));
    }
    ...
    await mkDeal({
        ...
        description: '7" pizza with 2 toppings (1 meat & 1 veg) and 1 soft drink (250ml) for only Rs 549. Monday–Friday 12–4pm.',
        price: 549,
        lunch: true,
        slots: [
            { type: 'choice_list', sourceMenuItemIds: [byo.id], quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink250Ids, quantity: 1, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        description: 'Any Large 12" Classic pizza for only Rs 999! Monday–Friday 12–4pm.',
        price: 999,
        lunch: true,
        slots: [{ type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true }],
    });

    // Medium Pizza/Pasta Lunch Offer — 9"/medium pizza or pasta for Rs899.
    await mkDeal({
        name: 'Medium Pizza, Pasta Lunch Offer',
        description: '9" pizza with 2 toppings (1 meat & 1 veg) or a pasta for only Rs 899. Monday–Friday 12–4pm.',
        price: 899,
        lunch: true,
        slots: [{ type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true }],
    });
    ...
    await mkDeal({
        ...
        description: 'Choose any wrap and a 250ml drink for Rs 549 (upgrade to Firey Special wrap for +Rs 100). Monday–Friday 12–4pm.',
        price: 549,
        lunch: true,
        slots: [
            {
                ...
                slotSurcharges: firySpecialWrap ? { [String(firySpecialWrap.id)]: 100 } : null,
            },
            { type: 'choice_list', sourceMenuItemIds: drink250Ids, quantity: 1, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        name: 'Snack Special',
        description: '1 Cheesy Garlic Bread, 1 portion of 5 pcs chicken wings or strips, and 2 dips.',
        price: 1299,
        slots: [
            { type: 'fixed', sourceMenuItemId: cheesyGarlicBread.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: [wings.id, strips.id], quantity: 1, allowCustomization: true },
            { type: 'fixed', sourceMenuItemId: twoDips.id, quantity: 1, allowCustomization: true },
        ],
    });
    ...
    await mkDeal({
        ...
        description: '1 any Small (7") Classic or Signature pizza, 1 soft drink (345ml).',
        price: 725,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink345Ids, quantity: 1, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        description: '1 any Medium (9"/10") Classic or Signature pizza, 2 soft drinks (345ml).',
        price: 1399,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink345Ids, quantity: 2, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        description: '1 any Large (12") Classic or Signature pizza, 1 soft drink (1L).',
        price: 1899,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink1LIds, quantity: 1, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        description: '1 any Large Classic or Signature pizza, 1 Cheesy Garlic Bread or 5 wings or 5 strips, and 2 soft drinks (345ml).',
        price: 2499,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: [cheesyGarlicBread.id, wings.id, strips.id], quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink345Ids, quantity: 2, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        price: 1049,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPasta.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink345Ids, quantity: 1, allowCustomization: false },
        ],
    });
    ...
    await mkDeal({
        ...
        description: '1 pasta, 1 Cheesy Garlic Bread, and 1 soft drink (345ml).',
        price: 1499,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPasta.id, quantity: 1, allowCustomization: true },
            { type: 'fixed', sourceMenuItemId: cheesyGarlicBread.id, quantity: 1, allowCustomization: true },
    ...
    await mkDeal({
        ...
        description: '1 any Large Classic or Signature pizza, 1 Chocolate Pizza, and 2 soft drinks (345ml).',
        price: 2699,
        slots: [
            { type: 'choice_category', sourceCategoryId: catPizza.id, quantity: 1, allowCustomization: true },
            { type: 'fixed', sourceMenuItemId: chocPizza.id, quantity: 1, allowCustomization: true },
            { type: 'choice_list', sourceMenuItemIds: drink345Ids, quantity: 2, allowCustomization: false },
        ],
    });
    ...
    for (const b of branches) {
        for (const it of allItems) {
            const existing = await bmiRepo.findOne({ where: { branchId: b.id, menuItemId: it.id } });
            if (!existing) {
                await bmiRepo.save(
                    bmiRepo.create({ branchId: b.id, menuItemId: it.id, priceOverride: null, isAvailable: true, isHiddenOnline: false }),
                );
            }
        }
    }
