...
const BankCardBinLookup: React.FC<{ branchId?: number | null; compact?: boolean }> = ({
  ...
  return (
    <div className={compact ? '' : 'rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-600 dark:bg-slate-800/60'}>
      {!compact && (
        <p className="mb-2 text-sm font-semibold text-gray-700 dark:text-slate-200">
          Check a customer&apos;s card — enter the first digits (BIN)
        </p>
      )}
      <div className="flex gap-2">
        <input
          ...
          aria-label="Card BIN"
          className="w-full max-w-[220px] rounded-xl border-2 border-gray-200 px-3 py-2 text-sm text-gray-800 outline-none focus:border-red-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
        />
        <button
          type="button"
          onClick={() => void search()}
          disabled={!canSearch}
          className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-gray-700 disabled:opacity-40 dark:bg-slate-200 dark:text-slate-900 dark:hover:bg-white"
        >
