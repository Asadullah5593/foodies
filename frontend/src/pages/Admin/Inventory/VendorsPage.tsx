  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {canCreate && <button onClick={openCreate} className="inline-flex items-center gap-2 rounded-lg bg-red-600 hover:bg-red-700 text-white px-4 py-2.5 text-sm font-semibold shadow-sm"><LuPlus className="w-4 h-4" /> Add vendor</button>}
      </div>
            ...
            <tbody className="text-slate-700 dark:text-slate-200">
              {rows.length === 0 ? <tr><td colSpan={5} className="py-12 text-center text-slate-400">No vendors found.</td></tr> : rows.map((v) => (
                <tr key={v.id} className="border-t border-slate-100 dark:border-slate-700">
                  ...
                  <td className="py-3 px-4"><div className="flex gap-2 justify-end">{canEdit && <button onClick={() => openEdit(v)} className="px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700"><LuPencil className="w-3.5 h-3.5" /></button>}{canDelete && <button onClick={() => { if (confirm(`Delete vendor "${v.name}"?`)) deleteM.mutate(Number(v.id)); }} className="px-2.5 py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50"><LuTrash2 className="w-3.5 h-3.5" /></button>}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
